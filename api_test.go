...
// Test the PacketScanner functionality
func TestPacketScanner(t *testing.T) {
	src, err := os.Open("SkypeIRC.cap")
	if err != nil {
		t.Error("Missing pcap file.")
	}
	ps, err := NewPacketScanner(src)
	if err != nil {
		t.Error(err)
	}
	...
	if !ps.Next() {
		if ps.Error() != nil {
			...
			t.Error("Could not read packet from file; probably empty file")
		}
	}
